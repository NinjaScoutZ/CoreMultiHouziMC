...
public class RGBData {
	...
	public RGBData(int red, int green, int blue) {
		_red = Math.max(0, Math.min(1, ((double) red) / 255.d));
		_green = Math.max(0, Math.min(1, ((double) green) / 255.d));
		_blue = Math.max(0, Math.min(1, ((double) blue) / 255.d));
	}
	...
	public String toString() {
		return "RGB[" + "red=" + (int) (_red * 255) + ", " + "green=" + (int) (_green * 255) + ", " + "blue="
				+ (int) (_blue * 255) + "]";
	}
