...
{
    public static HashMap<Integer, ItemStack> removeItem(Inventory inventory, int endingSlot, ItemStack... items) 
    {
    ...
    public static int first(Inventory inventory, int endingSlot, ItemStack item, boolean withAmount) 
    {        
        if (endingSlot >= 54)
            return -1;
        ...
        for (int i = 0; i < endingSlot; i++) 
        {
            ...
            boolean equals = item.isSimilar(contents[i]);

            if (equals && withAmount) 
            {
                equals = contents[i].getAmount() >= item.getAmount();
            }
	...
	public static int getCountOfObjectsRemoved(Inventory getInventory, int i, ItemStack itemStack) 
	{
		int count = 0;
	...
	public static int GetCountOfObjectsRemovedInSlot(Inventory getInventory, int slot, ItemStack itemStack) 
	{
		int count = 0;
