...
public class WinEffectPage extends GadgetPage {
    public WinEffectPage(CosmeticManager plugin, CosmeticShop shop, CoreClientManager clientManager,
    ...
    protected void buildPage() {
        ...
        if (getPlugin().getGadgetManager().getGadgets(GadgetType.WinEffect) != null) {
            gadgets.addAll(getPlugin().getGadgetManager().getGadgets(GadgetType.WinEffect));
        }

        int[] slots = getCenteredSlots(gadgets.size());
        for (int i = 0; i < gadgets.size() && i < slots.length; i++) {
            addGadget(gadgets.get(i), slots[i]);
            if (getPlugin().getGadgetManager().getActive(getPlayer(), GadgetType.WinEffect) == gadgets.get(i)) {
                addGlow(slots[i]);
            }
        }
