package com.houzicore.shared.core.fire;
...
import com.houzicore.shared.core.condition.Condition.ConditionType;
...
public class Fire extends MiniPlugin
{
	...
	public void IgniteCollide(UpdateEvent event)
	{
		...
		for (Item fire : _fire.keySet())
		{
			...
			{
				if (ent instanceof Player && (com.houzicore.shared.common.util.UtilPlayer.isSpectator(ent) || ((Player)ent).getGameMode() == GameMode.CREATIVE))
					continue;

				if (ent.hasPotionEffect(PotionEffectType.FIRE_RESISTANCE))
					continue;
				
				if (ent.getLocation().getBlock().getType() == Material.WATER || ent.getLocation().getBlock().getType() == Material.LAVA)
					continue;
	...
	public void IgnitePickup(PlayerPickupItemEvent event)
	{
		...
		if (com.houzicore.shared.common.util.UtilPlayer.isSpectator(player) || player.getGameMode() == GameMode.CREATIVE)
			return;
		
		if (player.hasPotionEffect(PotionEffectType.FIRE_RESISTANCE))
			return;
		
		if (player.getLocation().getBlock().getType() == Material.WATER || player.getLocation().getBlock().getType() == Material.LAVA)
			return;
	...
	public void RemoveNear(Location loc, double range)
	{
		...
		for (Item cur : remove)
		{
			...
			// Note: Effect.EXTINGUISH handles the fizzing block-extinguish sound in valid Paper environments
