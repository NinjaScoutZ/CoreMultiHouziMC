...
/**
 * StatEventsRepository offers the ability to insert and log newly generated
 * stat events.
 *
...
public class StatEventsRepository extends RepositoryBase {
	...
	private static String INSERT_EVENT = "INSERT INTO statEvents(accountId, gamemode, serverGroup, type, value, date) "
			+ "SELECT accounts.id, ?, ?, ?, ?, CURRENT_DATE() " + "FROM accounts WHERE name = ? "
			+ "ON DUPLICATE KEY UPDATE value=value+";
	 ...
	 * 
	 * @param plugin
	 *            - the plugin responsible for instantiating this repository.
	 */
	 ...
	 * 
	 * @param accountId
	 *            - the id of the account responsible for the stat event.
	 * @param gamemode
	 *            - the id of the gamemode type at the time of the stat event.
	 * @param serverGroup
	 *            - the server group id associated with the stat event.
	 * @param type
	 *            - the type of stat event to be inserted (id of type).
	 * @param value
	 *            - the integer based value denoting the actual statistic being
	 *            logged.
	 */
	public void insertStatEvent(String playerName, int gamemode, String serverGroup, int type, int value) {
		...
		executeUpdate(INSERT_EVENT + value + ";", new ColumnInt("gamemode", gamemode),
				new ColumnVarChar("serverGroup", 100, serverGroup), new ColumnInt("type", type),
				new ColumnInt("value", value), new ColumnVarChar("name", 100, playerName));
	}
