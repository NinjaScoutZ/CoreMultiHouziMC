...
public class KillEffectPage extends GadgetPage {
    public KillEffectPage(CosmeticManager plugin, CosmeticShop shop, CoreClientManager clientManager,
    ...
    protected void buildPage() {
        ...
        if (getPlugin().getGadgetManager().getGadgets(GadgetType.KillEffect) != null) {
            gadgets.addAll(getPlugin().getGadgetManager().getGadgets(GadgetType.KillEffect));
        }

        int[] slots = getCenteredSlots(gadgets.size());
        for (int i = 0; i < gadgets.size() && i < slots.length; i++) {
            addGadget(gadgets.get(i), slots[i]);
            if (getPlugin().getGadgetManager().getActive(getPlayer(), GadgetType.KillEffect) == gadgets.get(i)) {
                addGlow(slots[i]);
            }
        }
