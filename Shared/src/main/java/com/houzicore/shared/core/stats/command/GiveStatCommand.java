...
public class GiveStatCommand extends CommandBase<StatsManager> {
	...
	public void Execute(final Player caller, final String[] args) {
		...
		try {
			final Player player = UtilPlayer.searchOnline(caller, args[0], true);
			...
			if (player == null) {
				Plugin.getClientManager().loadClientByName(args[0], new Runnable() {
					...
					public void run() {
						...
						if (client != null) {
							Plugin.incrementStat(client.getAccountId(), statName,
									Long.parseLong(args[args.length - 1]));
						} else {
			...
			UtilPlayer.message(caller,
					F.main("Stats", "Applied " + F.elem(Integer.parseInt(args[args.length - 1]) + " " + statName)
							+ " to " + F.elem(player.getName()) + "."));
