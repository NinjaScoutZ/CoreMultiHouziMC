...
public abstract class SalesPackageBase implements ICurrencyPackage, IDisplayPackage {
	...
	private final byte _displayData;
	...
	public SalesPackageBase(String name, Material material, byte displayData, String[] description, int coins,
		...
		CurrencyCostMap.put(CurrencyType.Essence, coins);
		Quantity = quantity;
	}
	...
	public void setEssenceCost(int cost) {
		CurrencyCostMap.put(CurrencyType.Essence, cost);
	}
	...
	public void Update(GameSalesPackageToken token) {
		...
		if (token.Gems > 0) {
			CurrencyCostMap.put(CurrencyType.Essence, token.Gems);
		}
	}
}
