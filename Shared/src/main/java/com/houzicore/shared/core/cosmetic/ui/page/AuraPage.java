...
public class AuraPage extends GadgetPage {
    public AuraPage(CosmeticManager plugin, CosmeticShop shop, CoreClientManager clientManager,
    ...
    protected void buildPage() {
        ...
        if (getPlugin().getGadgetManager().getGadgets(GadgetType.Aura) != null) {
            gadgets.addAll(getPlugin().getGadgetManager().getGadgets(GadgetType.Aura));
        }

        int[] slots = getCenteredSlots(gadgets.size());
        for (int i = 0; i < gadgets.size() && i < slots.length; i++) {
            addGadget(gadgets.get(i), slots[i]);
            if (getPlugin().getGadgetManager().getActive(getPlayer(), GadgetType.Aura) == gadgets.get(i)) {
                addGlow(slots[i]);
            }
        }
