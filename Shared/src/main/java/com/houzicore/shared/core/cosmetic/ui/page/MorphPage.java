...
public class MorphPage extends GadgetPage {
	public MorphPage(CosmeticManager plugin, CosmeticShop shop, CoreClientManager clientManager,
	...
	protected void buildPage() {
		...
		if (getPlugin().getGadgetManager().getGadgets(GadgetType.Morph) != null) {
			gadgets.addAll(getPlugin().getGadgetManager().getGadgets(GadgetType.Morph));
		}

		int[] slots = getCenteredSlots(gadgets.size());
		for (int i = 0; i < gadgets.size() && i < slots.length; i++) {
			addGadget(gadgets.get(i), slots[i]);
			if (getPlugin().getGadgetManager().getActive(getPlayer(), GadgetType.Morph) == gadgets.get(i)) {
				addGlow(slots[i]);
			}
		}
