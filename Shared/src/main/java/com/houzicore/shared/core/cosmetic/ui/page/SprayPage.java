...
public class SprayPage extends GadgetPage {
    public SprayPage(CosmeticManager plugin, CosmeticShop shop, CoreClientManager clientManager,
    ...
    protected void buildPage() {
        ...
        if (getPlugin().getGadgetManager().getGadgets(GadgetType.Spray) != null) {
            gadgets.addAll(getPlugin().getGadgetManager().getGadgets(GadgetType.Spray));
        }

        int[] slots = getCenteredSlots(gadgets.size());
        for (int i = 0; i < gadgets.size() && i < slots.length; i++) {
            addGadget(gadgets.get(i), slots[i]);
            if (getPlugin().getGadgetManager().getActive(getPlayer(), GadgetType.Spray) == gadgets.get(i)) {
                addGlow(slots[i]);
            }
        }
