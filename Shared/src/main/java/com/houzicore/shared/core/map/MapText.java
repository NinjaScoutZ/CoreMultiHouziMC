...
public class MapText {
	...
	public ItemStack getMap(boolean sendToServer, String... text) {
		...
		for (final String string : text) {
			for (final String line : split(string)) {
				int length = 1;

				for (final char c : line.toCharArray()) {
					final BufferedImage img = _characters.get(c);
		...
		item.setDurability((short) map.getId());
	...
	private void loadCharacters() {
		try {
			final InputStream inputStream = getClass().getResourceAsStream("ascii.png");
			final BufferedImage image = ImageIO.read(inputStream);

			final char[] text = new char[] { ' ', '!', '"', '#', '$', '%', '&', '\'', '(', ')', '*', '+', ',', '-', '.',
