package com.houzicore.shared.serverdata.servers;
...
import com.houzicore.shared.serverdata.redis.RedisConfig;
import com.houzicore.shared.serverdata.redis.RedisServerRepository;
import com.houzicore.shared.serverdata.servers.ConnectionData.ConnectionType;
...
{	
	...
	// Redis enabled toggle — loaded from houzicore-database.properties
	private static Boolean _redisEnabled = null;
	...
	/**
	 * @return whether Redis is enabled via the config file. Defaults to false.
	 */
	public static boolean isRedisEnabled()
	{
		if (_redisEnabled == null)
		{
			loadRedisConfig();
		}
	...
	/**
	 * Load the redis.enabled flag from houzicore-database.properties.
	 */
	private static void loadRedisConfig()
	{
		...
		try
		{
			File propsFile = new File("houzicore-database.properties");
			if (propsFile.exists())
			{
				...
				_redisEnabled = Boolean.parseBoolean(props.getProperty("redis.enabled", "false"));
				log("Redis enabled: " + _redisEnabled);
			}
			else
			{
				log("houzicore-database.properties not found — Redis disabled by default.");
			}
		}
		catch (Exception e)
		{
			log("Error reading redis config: " + e.getMessage() + " — Redis disabled by default.");
		}
	}
	...
	public static RedisConfig getConfig()
	{
		if (_config == null)
		{
			...
			{
				System.out.println("[ServerManager] Error: " + exception.getMessage());
