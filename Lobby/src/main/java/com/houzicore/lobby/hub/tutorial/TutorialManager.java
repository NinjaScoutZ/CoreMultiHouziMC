...
import com.houzicore.lobby.hub.tutorial.types.PartyTutorial;
import com.houzicore.lobby.hub.tutorial.types.WelcomeTutorial;
...
public class TutorialManager extends MiniPlugin
{
	...
	public TutorialManager(HubManager manager, DonationManager donation, TaskManager task, TextManager text) 
	{
		...
		// WelcomeTutorial removed - contained legacy branding
	...
	public void EntityInteract(PlayerInteractEntityEvent event)
	{
		...
		for (Tutorial tut : _tutorials)
		{
			...
			{
				UtilPlayer.message(event.getPlayer(), F.main("Tutorial", LangManager.get().get(event.getPlayer(), "tutorial.started", tut.GetTutName())));
			...
			while (tuteIterator.hasNext())
			{
				...
				if (data.Update())
				{
					...
					else
					{
						...
						//Inform
						UtilPlayer.message(player, F.main("Tutorial", LangManager.get().get(player, "tutorial.completed", tut.GetTutName())));
								...
								public void run(Boolean completed)
								{
									_donationManager.RewardEssence(new Callback<Boolean>() 
									{
										public void run(Boolean completed)
										{
											if (completed)
											{
												UtilPlayer.message(player, F.main("Tutorial", LangManager.get().get(player, "tutorial.reward", tut.GetEssence())));
												
												//Sound
												player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 2f, 1.5f);
											}
										}
									}, "Tutorial " + tut.GetTutName(), player.getName(), player.getUniqueId(), tut.GetEssence());									
								}
							}, player, tut.GetTask());
						}
					}
