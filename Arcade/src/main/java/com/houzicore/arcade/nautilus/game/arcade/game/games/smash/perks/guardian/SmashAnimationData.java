...
public class SmashAnimationData
{
	...
	public void update()
	{
		...
		if (_ticks % 5 == 0)
		{
			...
			{
				if (blockRestore.Contains(block) || UtilBlock.airFoliage(block) || block.isLiquid())
				{
					continue;
				}
				
				blockRestore.Add(block, IdUtil.getTypeId(Material.PRISMARINE), (byte) random.nextInt(3), (long) (_duration + 5000 * Math.random()));
			}
			...
			for (Player player : UtilPlayer.getNearby(_target.getLocation(), DAMAGE_RADIUS))
			{
				...
				Location end = _target.getEyeLocation();
				double distance = start.distance(end);
				Vector dir = end.toVector().subtract(start.toVector()).normalize();
				for (double d = 0; d < distance; d += 0.2)
				{
					Location pLoc = start.clone().add(dir.clone().multiply(d));
					pLoc.getWorld().spawnParticle(org.bukkit.Particle.WITCH, pLoc, 1, 0, 0, 0, 0);
				}
				
				player.getWorld().playSound(player.getLocation(), Sound.ENTITY_ZOMBIE_VILLAGER_CURE, 1, 0.75F);
				UtilAction.velocity(player, vector, 0.7, true, player.getLocation().getY() - 3 > _target.getLocation().getY() ? 0 : 0.5, 0, 10, true);
