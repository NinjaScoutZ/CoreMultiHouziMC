...
// Manager.getDamager().AddThrow(bone, player, this, -1, true, true, true, false, 0.5f);
			}
		}		
	}
   ...
   // /* event.AddKnockback(...) */, 10);
		
		}
		if (event.getCause().name() != null && event.getCause().name().contains("Bone Storm")) {
			// ; // /* event.AddKnockback(...) */, 6); // not migrated
		}
	}
