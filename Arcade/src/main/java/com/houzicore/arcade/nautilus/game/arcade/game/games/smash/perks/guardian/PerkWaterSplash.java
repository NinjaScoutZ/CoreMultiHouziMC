...
public class PerkWaterSplash extends Perk
{
	...
	public void activate(PlayerInteractEvent event)
	{
		...
		player.setVelocity(new Vector(0, _velocityY, 0));
		...
		for (Player other : UtilPlayer.getNearby(player.getLocation(), _radius))
		{
			...
			other.setVelocity(UtilAlg.getTrajectory(other, player).setY(0.5));
		}
	}
	...
	public void update(UpdateEvent event)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (UtilEnt.isGrounded(player) && !player.getLocation().getBlock().getRelative(BlockFace.DOWN).getType().name().contains("FENCE") && UtilTime.elapsed(_active.get(uuid), _minAirTime))
			{
				iterator.remove();
				_usedSecondBoost.remove(uuid);
				...
				player.getWorld().playSound(player.getLocation(), Sound.ENTITY_GENERIC_SPLASH, 2, 0);
