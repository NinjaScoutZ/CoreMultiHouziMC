package com.houzicore.arcade.nautilus.game.arcade.game.games.smash.perks.skeleton;
...
public class PerkBarrage extends SmashPerk
{
	...
	public void BarrageDrawBow(PlayerInteractEvent event)
	{
		...
		if (!UtilGear.isBow(player.getInventory().getItemInMainHand()))
		{
			return;
		}
	...
	public void BarrageCharge(UpdateEvent event)
	{
		...
		for (Player cur : UtilServer.getPlayers())
		{
			...
			if (cur.getInventory().getItemInMainHand() == null || cur.getInventory().getItemInMainHand().getType() != Material.BOW)
			{
				if (_useExp)
				{
					cur.setExp(0f);
			...
			// Effect
			cur.playSound(cur.getLocation(), Sound.UI_BUTTON_CLICK, 1f, 1f + (0.1f * _charge.get(key)));
		}
	}
	...
	public void BarrageArrows(UpdateEvent event)
	{
		...
		for (UUID key : _firing)
		{
			...
			_arrows.add(arrow);
			cur.getWorld().playSound(cur.getLocation(), Sound.ENTITY_ARROW_SHOOT, 1f, 1f);
		}
