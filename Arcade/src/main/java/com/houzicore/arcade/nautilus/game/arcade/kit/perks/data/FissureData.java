...
public class FissureData 
{
	...
	public FissureData(Perk host, Player player, Vector vec, Location loc)
	{
		Host = host;
	...
	{	
		while (UtilMath.offset2d(_loc, _startLoc) < 14) 
		{
			...
			//Effect
			_loc.getWorld().playEffect(block.getLocation(), Effect.STEP_SOUND, IdUtil.getTypeId(block));
	...
	public boolean Update() 
	{
		...
		{
			_loc.getWorld().playEffect(up.getLocation(), Effect.STEP_SOUND, IdUtil.getTypeId(up));
			_height = 0;
			_handled++;
			return false;
		}
		...
		//Boost Column
		if (IdUtil.getTypeId(block) == 1)		Host.Manager.GetBlockRestore().Add(block, 4, IdUtil.getData(block), 14000);
		if (IdUtil.getTypeId(block) == 2)		Host.Manager.GetBlockRestore().Add(block, 3, IdUtil.getData(block), 14000);
		if (IdUtil.getTypeId(block) == 98)	Host.Manager.GetBlockRestore().Add(block, 98, (byte)2, 14000);

		Host.Manager.GetBlockRestore().Add(up, IdUtil.getTypeId(block), IdUtil.getData(block), 10000 - (1000 * _height));
		_height++;

		//Effect
		up.getWorld().playEffect(up.getLocation(), Effect.STEP_SOUND, IdUtil.getTypeId(block));
