...
public class PerkWitherSkull extends SmashPerk
{
	...
	private static final ItemStack HELMET = new ItemStack(Material.WITHER_SKELETON_SKULL, 1);
	...
	public void activate(PlayerInteractEvent event)
	{
		...
		if (!smash && (!player.getInventory().getItemInMainHand().getType().name().endsWith("_SWORD") || !UtilEvent.isAction(event, ActionType.R)))
		{
			return;
		}
	...
	public void cleanAndControl(UpdateEvent event)
	{
		...
		_active.removeIf(skullData ->
		{
			...
			UtilParticle.PlayParticleToAll(ParticleType.LARGE_SMOKE, newLocation.add(0, 2, 0), 0f, 0f, 0f, 0.01F, 2, ViewDist.LONG);
	...
	private void explode(SkullData skullData, boolean adjustY)
	{
		...
		UtilParticle.PlayParticleToAll(ParticleType.HUGE_EXPLOSION, location, 0f, 0f, 0f, 0f, 1, ViewDist.LONG);
		skullHolder.remove();
	}
