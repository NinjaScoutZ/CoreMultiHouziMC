...
import com.houzicore.shared.recharge.*;
...
public class PerkDestructor extends Perk
{
	...
	public void collide(PlayerTeleportEvent event)
	{
		...
		event.getTo().getWorld().playSound(event.getTo(), Sound.ENTITY_GENERIC_EXPLODE, 1f, 0.5f);
		event.getTo().getWorld().playSound(event.getTo(), Sound.ENTITY_FIREWORK_ROCKET_TWINKLE, 2f, 0.5f);
		
		// Apply Wither effect to nearby enemies
		for (Player p : Manager.GetGame().GetPlayers(true))
		{
			if (p.equals(event.getPlayer()) || !Manager.GetGame().IsAlive(p))
				continue;
				
			if (com.houzicore.shared.common.util.UtilMath.offset(p.getLocation(), event.getTo()) < 5)
			{
