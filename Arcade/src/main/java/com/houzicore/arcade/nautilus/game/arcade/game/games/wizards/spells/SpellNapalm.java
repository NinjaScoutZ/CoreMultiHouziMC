...
public class SpellNapalm extends Spell implements SpellClick
{
	...
	public SpellNapalm()
	{
		...
		_glazedBlocks.put(Material.GRASS_BLOCK, Material.DIRT);
		_glazedBlocks.put(Material.OAK_FENCE, Material.NETHER_BRICK_FENCE);
		_glazedBlocks.put(Material.OAK_STAIRS, Material.NETHER_BRICK_STAIRS);
		_glazedBlocks.put(Material.STONE_BRICK_STAIRS, Material.NETHER_BRICK_STAIRS);
		_glazedBlocks.put(Material.SAND, Material.GLASS);
		_glazedBlocks.put(Material.STONE_BRICKS, Material.NETHER_BRICK);
		_glazedBlocks.put(Material.OAK_LOG, Material.NETHERRACK);
		_glazedBlocks.put(Material.ACACIA_LOG, Material.NETHERRACK);
		_glazedBlocks.put(Material.STONE_BRICKS, Material.COBBLESTONE);
		_glazedBlocks.put(Material.CLAY, Material.WHITE_TERRACOTTA);
	}
	...
	public void castSpell(final Player player)
	{
		...
		new BukkitRunnable()
		{
			...
			public void run()
			{
							...
							for (int x = -bSize; x <= bSize; x++)
							{
								for (int z = -bSize; z <= bSize; z++)
								{
									...
									if (heat > 0)
									{
										...
										{
											float strength = block.getType().getHardness() * 0.7F;
											...
											if (strength <= heat)
											{
												...
												block.getWorld().playSound(block.getLocation(), Sound.BLOCK_FIRE_EXTINGUISH, 1.3F,
										...
										if (block.getType() == Material.WATER || block.getType() == Material.WATER)
										{
											if (heat > 1)
											{
												...
												block.getWorld().playSound(block.getLocation(), Sound.BLOCK_FIRE_EXTINGUISH, 1.3F, 0);
											...
											if (UtilMath.random.nextBoolean())
											{
												for (int a = 0; a < 6; a++)
												{
													...
													if (b.getType() != Material.AIR)
													{
														...
														block.getWorld().playSound(block.getLocation(), Sound.BLOCK_WOOL_BREAK, 1.3F,
																0.6F + ((new Random().nextFloat() - 0.5F) / 3F));
				...
				if (lastTick++ % 8 == 0)
				{
					napalmLoc.getWorld().playSound(napalmLoc, Sound.ENTITY_CAT_HISS, Math.min(0.8F + (float) (size * 0.09F), 1.8f), 0F);
				}
