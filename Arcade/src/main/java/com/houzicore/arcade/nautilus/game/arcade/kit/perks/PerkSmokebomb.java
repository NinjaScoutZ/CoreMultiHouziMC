...
//import com.houzicore.shared.condition.Condition;
...
//			Manager.GetCondition().Factory().Blind(GetName() + " Effect", living, player, getEffectDuration(), 0, false, false, true);
		...
		//Effects
		player.getWorld().playSound(player.getLocation(), Sound.BLOCK_FIRE_EXTINGUISH, 2f, 0.5f);
		UtilParticle.PlayParticle(ParticleType.HUGE_EXPLOSION, player.getLocation(), 0f, 0f, 0f, 0, 1,
				ViewDist.MAX, UtilServer.getPlayers());
	}

	@EventHandler(priority = EventPriority.HIGH)
	public void EndDamager(EntityDamageByEntityEvent event)
	{
...
//
//			if (!cond.getCause().name().equals(GetName()))
//				// continue;
