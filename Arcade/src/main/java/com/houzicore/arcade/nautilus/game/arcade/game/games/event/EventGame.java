...
import com.houzicore.arcade.nautilus.game.arcade.game.games.event.kits.KitPlayer;
...
public class EventGame extends Game
{
	...
	public void doubleJumpTrigger(PlayerToggleFlightEvent event)
	{
		...
		// if (disguise != null && ((disguise instanceof DisguiseChicken && !((DisguiseChicken)disguise).isBaby()) || ...))
		//	return; // disguise API not available
	...
	public void gadgetDisable(GadgetActivateEvent event)
	{
		...
		{
			event.setCancelled(true);
			event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 0.5f, 0.5f);
