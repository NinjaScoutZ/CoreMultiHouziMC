...
public class BedwarsIslandModule extends BedwarsModule
{
	...
	public void updateSpawn(UpdateEvent event)
	{
		...
		_islands.put(random, new BedwarsIsland(blocks));
		...
		_game.Announce(F.main("Game", "A new " + F.name(C.cGreen + "Island") + " has spawned! Find it and claim the loot!"));
	}
		...
		_islands.forEach((location, island) ->
		{
			...
			else if (island.getChestOpen() > 0 && UtilTime.elapsed(island.getChestOpen(), ISLAND_CRUMBLE_DELAY))
			{
				for (Player player : _game.GetPlayers(true))
				{
					...
					{
						player.playSound(player.getLocation(), Sound.ENTITY_HORSE_ARMOR, 1, 0.6F);
	...
	private Location getRandomLocation()
	{
		...
		{
			attempts++;
			Location location = getRandomLocation(_center, MAX_XZ, 0, MAX_XZ);
	...
	private Location getRandomLocation(Location center, double radiusX, double radiusY, double radiusZ)
	{
