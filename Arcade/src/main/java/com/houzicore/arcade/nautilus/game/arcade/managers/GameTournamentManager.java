...
public class GameTournamentManager implements Listener
{
	...
	public void pointsReward(GameStateChangeEvent event)
	{
		...
		if (event.GetGame().GetTeamList().isEmpty()) return;
	...
	public void pointsDisplay(GameStateChangeEvent event)
	{
		...
		com.houzicore.shared.common.util.UtilServer.broadcast(C.cBlue + C.Strike + C.Bold + "────────────────────────────────────────────");
		
		for (int i=0 ; i<ranks.size() ; i++)
		{
			com.houzicore.shared.common.util.UtilServer.broadcast(C.cGold + getTournamentPoints(ranks.get(i)) + " Points" + "   " + C.cWhite + ranks.get(i));
		}
