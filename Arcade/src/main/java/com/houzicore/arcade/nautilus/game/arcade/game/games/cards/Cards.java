...
import com.houzicore.arcade.nautilus.game.arcade.game.games.cards.kits.KitPlayer;
...
public class Cards extends SoloGame 
{
	...
	public Cards(ArcadeManager manager) 
	{
		super(manager, GameType.Cards,
				...
				new String[]
								{
				"[TH] Be creative and build something",
				"[TH] based on the build theme!"
								});
	...
	public void ParseData()
	{
		...
		if (!WorldData.GetDataLocs("PINK").isEmpty()) _questionLoc = WorldData.GetDataLocs("PINK").get(0);
	...
	public void prepare(PlayerPrepareTeleportEvent event)
	{
		...
		_voteRooms.put(event.GetPlayer(), new VoteRoom(
				(!WorldData.GetDataLocs("YELLOW").isEmpty() ? WorldData.GetDataLocs("YELLOW").remove(0) : null), 
