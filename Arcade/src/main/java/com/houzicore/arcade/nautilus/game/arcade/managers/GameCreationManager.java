...
public class GameCreationManager implements Listener
{
	...
	public void NextGame(UpdateEvent event)
	{
		...
		{
			// Only start a new game cycle when at least one player is online.
			// This prevents the Vote→Loading→Recruit cycle from running on an empty
			// server, which would destroy and respawn NPCs with nobody watching.
			...
			{ 
				...
				if (UtilTime.elapsed(game.GetStateTime(), 20000))
				{
					...
					for (Player player : game.WorldData.World.getPlayers())	
					{
						...
						// Paper 1.21: Cannot call Entity.remove() on Players - use kickPlayer only
...
//		Manager.GetAntiStack().SetEnabled(true);
		...
		if (Manager.getMapVotingManager() != null)
		{
			Manager.GetGame().SetState(GameState.Vote);
			Manager.getMapVotingManager().startVoting(Manager.GetGame());
		}
		else
		{
			Manager.GetGame().loadWorld(MapPref);
		}
	}
	...
	public void forceCreateGame(GameType gameType)
	{
		CreateGame(gameType);
	}
}
