package com.houzicore.arcade.nautilus.game.arcade.game.games.event;
...
import com.houzicore.arcade.nautilus.game.arcade.game.modules.DisableFallDamageModule;
...
public class EventModule extends MiniPlugin
{
	...
	{	
		...
		{
			DisableFallDamageModule mod = Manager.GetGame().getModule(DisableFallDamageModule.class);
			if (mod != null) {
				mod.unregister();
				Manager.GetGame().Announce(F.main("Event Settings", F.value("Damage Fall", F.tf(true))));
			} else {
				Manager.GetGame().registerModule(new DisableFallDamageModule(Manager.GetGame()));
				Manager.GetGame().Announce(F.main("Event Settings", F.value("Damage Fall", F.tf(false))));
			}
		}
	...
	private void listSettings(Player player) 
	{
		...
		UtilPlayer.message(player, F.value("Damage Fall", F.tf(Manager.GetGame().getModule(DisableFallDamageModule.class) == null)));
				...
				else if (arg.toLowerCase().charAt(0) == 'h' && arg.length() > 1)
				{
					try
					{
						...
						for (Entity ent : entSet)
						{
							...
							{
								LivingEntity cEnt = (LivingEntity)ent;
				...
				else if (arg.toLowerCase().charAt(0) == 'e' && arg.length() > 1)
				{
					try
					{
						...
						for (Entity ent : entSet)
						{
							if (ent instanceof LivingEntity)
							{
								((LivingEntity)ent).addPotionEffect(new org.bukkit.potion.PotionEffect(potionType, Integer.MAX_VALUE, 0));
							}
						}
					}
					catch (Exception e)
				...
				else if (arg.toLowerCase().charAt(0) == 'i' && arg.length() > 1)
				{
					try
					{
						...
						for (Entity ent : entSet)
						{
							if (ent instanceof LivingEntity)
							{
								((LivingEntity)ent).getEquipment().setItemInMainHand(new org.bukkit.inventory.ItemStack(mat));
							}
						}
					}
					catch (Exception e)
	...
	private void commandEffect(Player caller, String[] args)
	{
		...
		if (args.length >= 5)
		{
			...
			if (args[1].equalsIgnoreCase("all"))
				Manager.GetGame().Announce(F.main("Effect", F.value("Applied Effect", type.getKey().getKey() + " " + (mult+1) + " for " + dur + "s")));
			else
				UtilPlayer.message(caller, F.main("Effect", "Applied " + type.getKey().getKey() + " " + (mult+1) + " for " + dur + "s for Targets."));
