...
public class PerkBarrage extends SmashPerk
{
    ...
    @EventHandler(priority = EventPriority.HIGH)
    public void BarrageDamageTime(EntityDamageByEntityEvent event)
    {
        if (!_noDelay)
            return;
        ...
        // Damage Event
        Manager.GetDamage().NewDamageEvent((LivingEntity) event.getEntity(), damager, null, DamageCause.THORNS, event.getDamage(), true,
