...
import com.houzicore.arcade.nautilus.game.arcade.events.PerkDoubleJumpEvent;
...
public class PerkArts extends SmashPerk
{
	...
	public void playerInteract(PlayerInteractEvent event)
	{
		...
		if (!itemStack.getType().name().endsWith("_SHOVEL") || !hasPerk(player) || !Recharge.Instance.use(player, GetName(), 100, false, false))
		{
			return;
		}
	...
	public void playerDropItem(PlayerDropItemEvent event)
	{
		...
		if (!hasPerk(player) || itemStack == null || itemStack.getType().name().endsWith("_BED"))
		{
			return;
		}
	...
	public void damage(CustomDamageEvent event)
	{
		...
		if (isSuperActive(damager))
		{
			...
			event.AddMod("Perfection", "Perfection", _attackDamageMod, false);
			return;
		}
		...
		{
			case ATTACK:
				event.AddMod(data.Type.getName(), data.Type.getName(), _attackDamageMod, false);
				value += _attackKBMod;
				break;
			case DEFENSE:
				event.AddMod(data.Type.getName(), data.Type.getName(), (double) _defenseDamageMod, false);
				value += _defenseKBMod;
				break;
			case SPEED:
				event.AddMod(data.Type.getName(), data.Type.getName(), (double) _speedDamageMod, false);
				value += _speedKBMod;
				break;
		}
