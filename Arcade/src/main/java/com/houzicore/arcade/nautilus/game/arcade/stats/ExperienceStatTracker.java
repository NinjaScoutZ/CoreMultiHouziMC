		...
		if (event.GetState() == Game.GameState.End)
		{
			...
			if (winners != null)
			{
				for (Player player : event.GetGame().GetPlayers(false))
				{
					//Tally Gems
					double gemExp = 0;
					for (String reason : event.GetGame().GetEssence(player).keySet())
					{
						if (reason.toLowerCase().contains("participation"))
							continue;
						
						EssenceData gem = event.GetGame().GetEssence(player).get(reason);
					...
					addStat(player, "XP", expGained, false, true);
