package com.houzicore.arcade.nautilus.game.arcade.game.games.snowfight;
...
import com.houzicore.shared.common.util.UtilSound;
...
import com.houzicore.arcade.nautilus.game.arcade.game.games.snowfight.kits.KitTactician;
import com.houzicore.arcade.nautilus.game.arcade.game.games.snowfight.kits.KitMedic;
import com.houzicore.arcade.nautilus.game.arcade.game.games.snowfight.kits.KitSportsman;
import com.houzicore.arcade.nautilus.game.arcade.kit.Kit;
import com.houzicore.arcade.nautilus.game.arcade.kit.SmashKit;
// Removed unused EntityFireball
...
public class SnowFight extends TeamGame
{
	...
	public SnowFight(ArcadeManager manager)
	{
		super(manager, GameType.SnowFight,
				...
				new String[]
								{
					"[TH] Just like... kill your enemies. with snow.",
					"[TH] Be careful if you are on Ice your body will freeze"
								});
		...
		this.BlockBreakAllow = new HashSet<>(Arrays.asList(Material.OAK_FENCE.ordinal()));
	...
	public void IceDamage(UpdateEvent event) {
		...
		for(Player player : GetPlayers(true)) 
		{
			if(IsOnIce(player))
			{
				Bukkit.getPluginManager().callEvent(new org.bukkit.event.entity.EntityDamageEvent(player, DamageCause.CUSTOM, 2.0D));
			}
		}
	...
	public void BlockDamage(BlockDamageEvent event)
	{
		...
		if (UtilInv.contains(player, "", Material.SNOWBALL, (byte) 0, 16))
			return;
		...
		if (GetKit(player) instanceof KitSportsman)
			UtilInv.insert(player, new ItemStack(Material.SNOWBALL));
	...
	// return;
	//
	// if (!UtilGear.isMat(player.getItemInHand(), Material.SNOWBALL))
	// return;
	//
	...
	// {
	// if (block.getType().ordinal() == 78 || UtilBlock.airFoliage(block))
	// {
	// //Build
	// if (block.getType().ordinal() == 78)
	// {
	// block.setType(Material.values()[78]) /* TODO metadata (byte)(block.getData() + 1) */
	// block.setType(Material.values()[80]) /* TODO metadata (byte)0 */;
	// }
	// else
	// {
	// block.setType(Material.values()[78]) /* TODO metadata (byte)0 */, Sound.BLOCK_SNOW_STEP, 1f,
	// 0.6f);
	//
	// //Use Snow
	...
	//
	// //Sound
	// player.getWorld().playSound(player.getLocation(), Sound.BLOCK_SNOW_STEP, 3f,
	// 1.5f);
	// }
	...
	// ItemStackFactory.Instance.CreateStack(Material.SNOWBALL, count));
	// else
	...
	private void SnowDecrease(Block block, int height)
	{
		...
		if (block.getType() == Material.SNOW)
			snowLevel = ((org.bukkit.block.data.type.Snow)block.getBlockData()).getLayers();
		...
		else
		{
			...
			snowData.setLayers(snowLevel - height);
	...
	public void Hit(ProjectileHitEvent event)
	{
		...
		Block block = proj.getLocation().getBlock();

		block.getWorld().playEffect(block.getLocation(), Effect.STEP_SOUND, 80);
		((Player) proj.getShooter()).playSound(((Player) proj.getShooter()).getLocation(), Sound.ENTITY_ARROW_HIT_PLAYER, 1, 1);
	}
	...
	private void summonMeteor(Location loc, float fireballSize)
	{
		...
		final FallingBlock fallingblock = loc.getWorld().spawnFallingBlock(loc, Material.ICE.createBlockData());
	...
	public void MeteorHit(EntityChangeBlockEvent event)
	{
		...
		if (projectile.hasMetadata("Meteor"))
		{
			...
			for(Player player : UtilServer.getPlayers()) {
				player.playSound(projectile.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1, 1);
			}
			...
			for(Entity player : projectile.getNearbyEntities(size, size, size))
			{
				if(player instanceof Player)
				{
					...
					Bukkit.getPluginManager().callEvent(new org.bukkit.event.entity.EntityDamageEvent(damagee, DamageCause.CUSTOM, damage));
				}
			}
	...
	public void Place(BlockPlaceEvent event)
	{
		if(event.getBlock().getRelative(BlockFace.DOWN).getType() == Material.ICE
				...
				|| event.getBlock().getRelative(BlockFace.DOWN).getType() == Material.OAK_FENCE)
		{
