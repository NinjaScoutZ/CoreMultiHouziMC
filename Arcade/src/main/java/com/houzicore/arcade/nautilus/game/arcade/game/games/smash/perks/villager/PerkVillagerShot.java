...
public class PerkVillagerShot extends SmashPerk implements IThrown
{
	...
	public void playerInteract(PlayerInteractEvent event)
	{
		...
		if (!itemStack.getType().name().endsWith("_HOE") || !hasPerk(player) || !Recharge.Instance.use(player, GetName(), _cooldown, true, true))
		{
			return;
		}
		...
		player.getWorld().playSound(location, Sound.ENTITY_FIREWORK_ROCKET_LARGE_BLAST, 1.5F, 1);
		...
		for (int i = 0; i < _bullets; i++)
		{
			...
			Manager.GetProjectile().AddThrow(item, player, this, 5000, true, true, false, null, 0f, 0f, particleType, reduceParticles ? UpdateType.FASTEST : UpdateType.TICK, _hitBox);
		}
