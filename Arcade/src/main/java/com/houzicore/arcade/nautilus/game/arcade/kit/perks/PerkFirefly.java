...
public class PerkFirefly extends SmashPerk
{
	...
	public void Update(UpdateEvent event)
	{
		...
		while (dataIterator.hasNext())
		{
			...
			if (!UtilTime.elapsed(data.Time, 1500) && !superActive)
			{
				...
				data.Player.getWorld().playSound(data.Player.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 0.2f, 0.6f);
			...
			else if (!UtilTime.elapsed(data.Time, 2500) || superActive)
			{
				...
				data.Player.getWorld().playSound(data.Player.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 0.6f, 1.2f);
	...
	@EventHandler
	public void FireflyDamage(EntityDamageByEntityEvent event)
	{
		...
		while (dataIterator.hasNext())
		{
			...
			if (!data.Player.equals(event.getEntity()))
				continue;
