package com.houzicore.arcade.nautilus.game.arcade.game.games.hideseek.lang;
...
public final class PropRushLang {

    private static final LegacyComponentSerializer LEGACY = LegacyComponentSerializer.legacySection();

    private static PropRushLang INSTANCE;

    private final MiniMessage mini;

    private PropRushLang() {
        this.mini = MiniMessage.miniMessage();
    }

    public static synchronized void init(Plugin plugin) {
        if (INSTANCE == null) {
            INSTANCE = new PropRushLang();
        }
    }

    public static synchronized PropRushLang ensureInitialized(Plugin plugin) {
        if (INSTANCE == null) {
            init(plugin);
        }
        return INSTANCE;
    ...
    public static synchronized PropRushLang get() {
        if (INSTANCE == null) {
            INSTANCE = new PropRushLang();
        }
        return INSTANCE;
    ...
    public String get(Player player, String key, TagResolver... tags) {
        return getFallback(player, key, tags);
    }

    public String getFallback(Player player, String key, TagResolver... tags) {
        String raw = lookupWithFallback(player, key);
        if (raw == null) {
            raw = "<red>[Missing: " + key + "]";
        }
        return LEGACY.serialize(deserialize(raw, tags));
    }
