...
public class SpellRainbowRoad extends Spell implements SpellClick
{
	...
	public void onUpdate(UpdateEvent event)
	{
		...
		while (itel.hasNext())
		{
			...
			if (entry.getValue() < System.currentTimeMillis())
			{
				...
				if (entry.getKey().getType() == Material.WHITE_STAINED_GLASS)
				{
	...
	private int makeRoad(Location loc, BlockFace face, int colorProgress)
	{
		...
		for (Block b : bs)
		{
			...
			b.setType(com.houzicore.shared.common.util.IdUtil.getMaterial(95,
					(byte) _rainbow[colorProgress++ % _rainbow.length]));

			b.getWorld().playEffect(b.getLocation(), Effect.STEP_SOUND, Material.WHITE_WOOL,
					com.houzicore.shared.common.util.IdUtil.getData(b));
		...
		if (playSound)
		{
			block.getWorld().playSound(block.getLocation(), Sound.ENTITY_ZOMBIE_VILLAGER_CONVERTED, 1.5F, 1);
		}
