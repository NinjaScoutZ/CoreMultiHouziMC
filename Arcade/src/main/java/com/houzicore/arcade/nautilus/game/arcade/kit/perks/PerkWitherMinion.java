...
public class PerkWitherMinion extends Perk 
{
	...
	public void ShootWeb(PlayerInteractEvent event)
	{
		...
		for (int i=0 ; i<2 ; i++)
		{
			...
			skel.getEquipment().setHelmet(ItemStackFactory.Instance.CreateStack(Material.PLAYER_HEAD, (byte)1, 1));
		...
		//Effect
		player.getWorld().playSound(player.getLocation(), Sound.ENTITY_WITHER_HURT, 2f, 0.6f);
	}
	
	@EventHandler(priority = EventPriority.LOWEST)
	public void witherFallCancel(EntityDamageByEntityEvent event)
	{
		if (event.isCancelled())
			return;
		...
		if (!_ents.contains(event.getEntity()))
			return;
