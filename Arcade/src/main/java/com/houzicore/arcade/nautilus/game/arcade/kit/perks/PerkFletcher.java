...
public class PerkFletcher extends Perk
{
	...
	{
		return ItemStackFactory.Instance.CreateStack(Material.ARROW, (byte)0, amount, F.item("Fletched Arrow"));
	}
	...
	public void FletchShootBow(EntityShootBowEvent event)
	{
		...
		for (int i=0 ; i<=8 ; i++)
			if (isFletchedArrow(player.getInventory().getItem(i)))
			{
				...
				// ((CraftArrow) event.getProjectile()).getHandle().fromPlayer = 0; // not migrated
