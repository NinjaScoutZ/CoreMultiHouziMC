...
public class PerkWaller extends Perk
{
	...
	public void onPlace(PlayerInteractEvent event)
	{
		...
		if (!UtilInv.IsItem(event.getItem(), Material.STONE_SHOVEL, (byte) 0))
			return;
		
		UtilInv.remove(event.getPlayer(), Material.STONE_SHOVEL, (byte) 0, 1);
	...
	private Set<Location> buildWall(final Location source, final float playerYaw)
	{
		...
		for (Location block : allWallBlocks)
		{
			...
			// block.getBlock().setData((byte) 0); // Block.setData removed in 1.21
