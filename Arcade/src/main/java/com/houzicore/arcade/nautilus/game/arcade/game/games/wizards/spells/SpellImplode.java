				...
				public void run()
				{
					if (effectedBlocks.size() > 0)
					{
						...
						block.getWorld().playSound(block.getLocation(),
								new Random().nextBoolean() ? Sound.BLOCK_GRAVEL_BREAK : Sound.BLOCK_GRASS_BREAK, 2,
							...
							for (int i = 0; i < 6; i++)
							{
								...
								if (UtilBlock.airFoliage(b))
								{
									UtilParticle.PlayParticle(
											...
											b.getLocation().add(0.5, 0.5, 0.5), 0f, 0f, 0f, 0f, 1,
					...
					else if (timesRan++ >= 20)
					{
						...
						{
							player.playSound(player == p ? p.getLocation() : centerLocation, Sound.ENTITY_ENDER_DRAGON_GROWL, 1.5F, 1.5F);
						}
