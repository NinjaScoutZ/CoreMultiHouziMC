...
public class PerkWolfPack extends Perk
{
	...
	public void CubStrikeHit(Player damager, LivingEntity damagee, Wolf wolf)
	{
		...
		//Sound
		damagee.getWorld().playSound(damagee.getLocation(), Sound.ENTITY_WOLF_AMBIENT, 1.5f, 2f);
	...
	{
		if (event.getCause().name() != null && event.getCause().name().contains("Cub Strike"))
		{
   // /* event.AddKnockback(...) */, 3);
		}
		
		if (event.getDamager() != null && event.getDamager() instanceof Wolf)
		{
   // /* event.AddKnockback(...) */, 3);
		}
	}
