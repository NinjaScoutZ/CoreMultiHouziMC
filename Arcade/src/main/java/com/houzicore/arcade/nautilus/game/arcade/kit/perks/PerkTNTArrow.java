...
public class PerkTNTArrow extends Perk
{
	...
	public void activate(PlayerInteractEvent event)
	{
		...
		//Effect
		player.getWorld().playSound(player.getLocation(), Sound.BLOCK_FIRE_EXTINGUISH, 2.5f, 2.0f);
	...
	public void hitEntityTrigger(EntityDamageByEntityEvent event)
	{
		...
		if (!_arrows.remove(event.getDamager()))
			return;

		event.setCancelled(true);

		Location loc = event.getEntity().getLocation().subtract(event.getDamager().getVelocity().normalize().multiply(0.1));
		
		trigger((Arrow)event.getDamager(), loc);
	}
	...
	public void trigger(Arrow arrow, Location loc)
	{
		...
		loc.getWorld().playSound(loc, Sound.ENTITY_GENERIC_EXPLODE, 2f, 0.75f);
	}
