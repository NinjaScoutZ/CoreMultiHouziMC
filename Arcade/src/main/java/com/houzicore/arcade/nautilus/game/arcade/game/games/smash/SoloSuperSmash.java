...
public class SoloSuperSmash extends SuperSmash
{
	...
	{
		super(manager, GameType.SuperSmash, new String[]
				{
	...
	public void ScoreboardUpdate(UpdateEvent event)
	{
		...
		if (GetPlayers(false).size() > 14)
		{
			...
			Scoreboard.Write(C.cGreen + C.Bold + "Players Alive");
			Scoreboard.Write(GetPlayers(true).size() + " ");
 
			Scoreboard.WriteBlank();
			Scoreboard.Write(C.cRed + C.Bold + "Players Dead");
		...
		else
		{
			...
			writeGroup(GetPlayers(minimise), player ->
			{
