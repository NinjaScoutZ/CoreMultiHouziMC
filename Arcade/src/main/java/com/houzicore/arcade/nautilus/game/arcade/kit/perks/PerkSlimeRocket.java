...
public class PerkSlimeRocket extends SmashPerk implements IThrown
{
	...
	public void EnergyUpdate(UpdateEvent event)
	{
		...
		for (Player player : Manager.GetGame().GetPlayers(true))
		{
			...
			updateSlimeDisguiseSize(player, size);
	...
	private void updateSlimeDisguiseSize(Player player, int size)
	{
		Manager.GetDisguise().getService().getActiveSession(player.getUniqueId())
				.map(session -> session.request())
				.filter(request -> "SLIME".equalsIgnoreCase(request.variantKey()))
				.filter(request -> !String.valueOf(size).equals(request.attributes().get("slimeSize")))
				.ifPresent(request -> applyUpdatedDisguise(player, request.withAttribute("slimeSize", String.valueOf(size))));
	}
		...
		while (chargeIterator.hasNext())
		{
			...
			if (player.isBlocking())
			{
				...
				else
				{
					...
					//Effect
					player.getWorld().playSound(player.getLocation(), Sound.ENTITY_SLIME_SQUISH, 0.5f, (float)(0.5 + 1.5*(elapsed/3d)));
...
// Manager.getDamager().AddThrow(slime, player, this, -1, true, true, true,
// null, 0, 0, null, 0, UpdateType.FASTEST, 1f);
  ...
  // /* event.AddKnockback(...) */, 3.0);
	...
	@EventHandler
	public void SlimeDamage(EntityDamageByEntityEvent event)
	{
