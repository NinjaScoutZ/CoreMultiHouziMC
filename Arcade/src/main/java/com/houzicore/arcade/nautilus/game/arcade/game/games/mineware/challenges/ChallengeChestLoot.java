	...
	public void generateRoom()
	{
		...
		for (int x = -7; x <= 7; x++)
		{
			for (int z = -7; z <= 7; z++)
			{
				...
				// b.setData((byte) UtilMath.r(16)); // Block.setData removed in 1.21
				...
				if (Math.abs(x) % 2 == 0 && Math.abs(z) % 2 == 0)
				{
					...
					Material[] validItems = {
						Material.DIRT, Material.COBBLESTONE, Material.OAK_PLANKS, Material.STONE, 
						Material.SAND, Material.GRAVEL, Material.BONE, Material.ROTTEN_FLESH, 
						Material.APPLE, Material.STICK, Material.STRING
					};

					for (int i = 0; i < inventory.getSize(); i++)
					{
						ItemStack item = new ItemStack(validItems[UtilMath.r(validItems.length)]);
