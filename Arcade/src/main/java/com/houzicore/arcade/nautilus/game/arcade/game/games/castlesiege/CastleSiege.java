package com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege;
...
import com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege.kits.KitHumanKnight;
import com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege.kits.KitHumanMarksman;
import com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege.kits.KitHumanPeasant;
import com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege.kits.KitUndeadArcher;
import com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege.kits.KitUndeadGhoul;
import com.houzicore.arcade.nautilus.game.arcade.game.games.castlesiege.kits.KitUndeadZombie;
import com.houzicore.arcade.nautilus.game.arcade.kit.Kit;
import com.houzicore.arcade.nautilus.game.arcade.kit.NullKit;
import com.houzicore.arcade.nautilus.game.arcade.stats.BloodThirstyStatTracker;
import com.houzicore.arcade.nautilus.game.arcade.stats.KingDamageStatTracker;
import com.houzicore.arcade.nautilus.game.arcade.stats.KingSlayerStatTracker;
import com.houzicore.arcade.nautilus.game.arcade.stats.WinAsTeamStatTracker;
...
public class CastleSiege extends TeamGame
{
	...
	public CastleSiege(ArcadeManager manager)
	{
		super(manager, GameType.CastleSiege,
				...
				new String[]
						{
								F.elem(C.cAqua + "[TH] Defenders") + C.cWhite + "[TH]  must defend the King.",
								F.elem(C.cAqua + "[TH] Defenders") + C.cWhite + "[TH]  win when the sun rises.",
								F.elem(C.cAqua + "[TH] Defenders") + C.cWhite + "[TH]  respawn as wolves.",
								"",
								F.elem(C.cRed + "[TH] Undead") + C.cWhite + "[TH]  must kill the King.",
								F.elem(C.cRed + "[TH] Undead") + C.cWhite + "[TH]  lose when the sun rises.",
	...
	@EventHandler
	public void HorseDamageCancel(EntityDamageByEntityEvent event)
	{
		...
		event.setCancelled(true) /* was setCancelled("Horse Team Damage") */;
	}
	...
	@EventHandler
	public void KingDamage(EntityDamageByEntityEvent event)
	{
		if (_king == null || !_king.isValid())
			return;

		if (!event.getEntity().equals(_king))
			return;

		event.setCancelled(true) /* was setCancelled("King Damage") */;
	...
	public void WriteScoreboard()
	{
		...
		else
		{
			...
			{} // Manager.GetCondition().Factory().Ignite("Sun Damage", player, player, 5, false, false); // TODO: condition factory not migrated
			}
		}
	...
	public void TNTWeakness(ExplosionPrimeEvent event)
	{
		...
		weakness.getWorld().playSound(weakness, Sound.ENTITY_GENERIC_EXPLODE, 16f, 0.8f);
	}
	...
	public void DefenderBlockPlace(org.bukkit.event.block.BlockPlaceEvent event)
	{
		...
		if (event.getBlockAgainst().getType() == Material.OAK_FENCE)
		{
			event.setCancelled(true);
