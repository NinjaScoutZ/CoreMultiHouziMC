...
public class NamedTeamsModule extends GameModule<Game>
{
	...
	public NamedTeamsModule(Game game)
	{
		super(game);
	}
	...
	public void generateCustomTeams(GameStateChangeEvent event)
	{
		...
		if (_teamPerSpawn)
		{
			for (Location location : spawns)
			{
				...
				for (Block block : UtilBlock.getInBoundingBox(location.clone().subtract(2, 0, 2), location.clone().add(2, 0, 2)))
				{
		...
		else
		{
			...
			int teams = getGame().Manager.GetPlayerFull() / playersPerTeam;
	...
	{
		// Create the team
		GameTeam team = new GameTeam(getGame(), _currentColor._names[_nameIndex], _currentColor._color, new ArrayList<>(spawns), true);
