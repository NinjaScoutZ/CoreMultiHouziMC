...
public class GameStatManager implements Listener
{
	...
	public void flushExitStats(Player player)
	{
		Long joinTime = _joinTimes.remove(player.getUniqueId());

		if (joinTime != null)
		{
			int timeInGame = (int) ((System.currentTimeMillis() - joinTime) / 1000);
			
			Manager.GetStatsManager().incrementStat(player, "Global.TimeInGame", timeInGame);
		}
	}
